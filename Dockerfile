...
RUN \
    --mount=type=bind,from=compile-image,source=/opt/warp/debs,target=/opt/warp/debs \
    dpkg -i debs/*.deb
#COPY --from=compile-image /opt/warp/debs ./debs
#RUN dpkg -i debs/*.deb

RUN \
    --mount=type=bind,from=compile-image,source=/opt/warp/wheel,target=/opt/warp/wheel \
